...
#ifdef VIRTUAL_CURSOR
SDL_Rect dstCrosshair = { WINDOW_W / 2, WINDOW_H / 2, 100,100 };
...
//Some values below were tweaked to work best with my PC with a wired xbox 360 controller, discuss and fix them later
#ifdef VIRTUAL_CURSOR
void interpolateController() {
    // Set mouse speed to 10 pixels per second at 60FPS
    const float mouse_speed = 500.0f / 60.0f;

    // Prepare a circular deadzone; 10% inner and 5% outer deadzone
    const float deadzone_inner = 0.5f;
    const float deadzone_outer = 1.0f - 0.05f;

    // Get axis position in [-1.0;+1.0]
    stick_x = SDL_GameControllerGetAxis(controller, SDL_CONTROLLER_AXIS_LEFTX) / (float)0x8000;
    stick_y = SDL_GameControllerGetAxis(controller, SDL_CONTROLLER_AXIS_LEFTY) / (float)0x8000;

    float squared_stick_distance = stick_x * stick_x + stick_y * stick_y;

    // Do mouse controller while stick is not in center
    if (squared_stick_distance > (deadzone_inner * deadzone_inner)) {
        // Get non-squared axis distance to center, which should be [deadzone_inner;sqrt(2.0)]
        float stick_distance = sqrtf(squared_stick_distance);
        ...
        if (controllerSupport && isButtonDown && gameFrame > -1 && gameFrame < 3) {
            setAngle();
        }
    }
}
